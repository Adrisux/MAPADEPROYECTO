...
public class crudCliente {
    ...
    public static boolean update(clsCliente cliente) {
        ...
        lstPar.add(new Parametro(2, cliente.getNombre()));
        lstPar.add(new Parametro(3, cliente.getDireccion())); 
        lstPar.add(new Parametro(4,cliente.getId_cliente()));
        try{
            res= AccesoDatos.ejecutaComando(sql, lstPar);
        }catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public static clsCliente findbyId(clsCliente ins) {
        clsCliente cliente = null;
        String sql = "SELECT id_cliente, ruc,nombre,direccion FROM public.cliente WHERE id_cliente=?";
        ArrayList<Parametro> lstPar = new ArrayList<>();
        lstPar.add(new Parametro(1, ins.getId_cliente()));
        try {
            ConjuntoResultado cres = AccesoDatos.ejecutaQuery(sql, lstPar);
            while (cres.next()) {
                cliente = new clsCliente();
